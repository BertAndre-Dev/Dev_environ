...
import {
  selectEnergyProviderAvailableModules,
  selectEnergyProviderEstateModulesError,
  selectEnergyProviderEstateModulesLoading,
  selectEnergyProviderModulesError,
  selectEnergyProviderModulesLoading,
} from "@/redux/slice/energy-provider/estate-mgt/energy-provider-estate-slice";

interface EnergyProviderEstateModulesFormProps {
...
export function EnergyProviderEstateModulesForm({
  estateId,
  estateName,
  initialModules,
  onSuccess,
  onCancel,
}: EnergyProviderEstateModulesFormProps) {
  ...
  const availableModules = useSelector(selectEnergyProviderAvailableModules);
  const companyModulesLoading = useSelector(selectEnergyProviderModulesLoading);
  const companyModulesError = useSelector(selectEnergyProviderModulesError);
  const estateModulesLoading = useSelector(selectEnergyProviderEstateModulesLoading);
  const estateModulesError = useSelector(selectEnergyProviderEstateModulesError);

  useEffect(() => {
    const seeded = Array.isArray(initialModules) ? [...initialModules] : [];
    setSelectedModules(seeded);
    dispatch(fetchEnergyProviderEstateModules());
    dispatch(fetchEnergyProviderEstateEnabledModules(estateId))
      .unwrap()
      .then((res) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await dispatch(
        updateEnergyProviderEstateModules({ id: estateId, modules: selectedModules }),
      ).unwrap();
