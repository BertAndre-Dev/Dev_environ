...
type SegmentKey = "activeBills" | "suspendedBills";
...
const SEGMENT_CONFIG: ReadonlyArray<{
  ...
  { key: "activeBills", label: "Active", fill: "#10B981" },
  { key: "suspendedBills", label: "Suspended", fill: "#F59E0B" },
];

function buildChartSlices(data: BillsSummaryData): ChartSlice[] {
...
type BillsSummaryChartProps = Readonly<{
...
export function BillsSummaryChart({
  data,
  loading = false,
  error = null,
  onRetry,
  className,
}: BillsSummaryChartProps) {
  const total = Number(data?.totalBills ?? 0);
  const showError = Boolean(error) && !data && !loading;
  const showEmpty = !loading && !error && Boolean(data) && total === 0;

  let subtitle = `${total.toLocaleString()} total bills`;
  if (loading && !data) {
    subtitle = "Loading…";
  } else if (total === 1) {
    subtitle = "1 total bill";
  }
...
function ChartBody({
  ...
  const totalBills = Math.max(0, Number(data.totalBills ?? 0));
  const activeBills = Math.max(0, Number(data.activeBills ?? 0));
  const slices = buildChartSlices(data);
  const activeRatio = totalBills > 0 ? activeBills / totalBills : 0;
  const activeLow = totalBills > 0 && activeRatio < 0.5;
    ...
    <div className={cn("space-y-4", loading && "opacity-60")}>
      <div className="flex flex-col items-center gap-6 sm:flex-row sm:items-center">
        <div className="relative h-[220px] w-full max-w-[220px] shrink-0">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              ...
              <Tooltip content={<ChartTooltip segmentTotal={totalBills} />} />
            </PieChart>
          </ResponsiveContainer>
        ...
        <div className="flex w-full flex-col gap-2 sm:flex-1">
          {SEGMENT_CONFIG.map(({ key, label, fill }) => (
            <div
              key={key}
      ...
      >
        <div className="min-w-0">
          ...
          <p className="mt-0.5 text-xs">
            {activeLow
              ? "Fewer than half of bills are currently active."
              : "Bills currently in an active state."}
          </p>
        </div>
        <p
          className={cn(
        ...
        >
          {activeBills.toLocaleString()} / {totalBills.toLocaleString()}
        </p>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex h-[280px] flex-col items-center justify-center gap-2 text-center">
      ...
      <p className="text-xs text-muted-foreground">
        Bill status breakdown will appear when bills are created.
      </p>
    </div>
  );
}
...
function ErrorState({
  ...
  return (
    <div className="flex h-[280px] flex-col items-center justify-center gap-3 text-center">
      ...
      <p className="font-medium text-foreground">Couldn’t load bills summary</p>
...
export default BillsSummaryChart;
