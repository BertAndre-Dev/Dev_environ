...
export interface AddRevenueDraftEntry {
  id: string;
  description: string;
  amount: string;
...
export interface AddRevenueModalProps {
  open: boolean;
  saving: boolean;
  headName: string;
  drafts: AddRevenueDraftEntry[];
      ...
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto min-h-[30vh]">
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            <label className="text-sm font-medium" htmlFor="add-revenue-head">
              Revenue Head
            </label>
            <Input id="add-revenue-head" value={headName} disabled />
          </div>

          {showDateAndUpload ? (
            <div className="space-y-2">
              <label className="text-sm font-medium" htmlFor="add-revenue-date">
                Date
              </label>
              <IsoDatePicker
                id="add-revenue-date"
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between gap-3">
              <p className="text-sm font-medium">Revenue entries</p>
              <Button
                type="button"
                variant="outline"
                size="sm"
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <div className="space-y-2">
                    <label
                      ...
                      htmlFor={`add-revenue-desc-${idx}`}
                    >
                      Description
                    </label>
                    <Input
                      id={`add-revenue-desc-${idx}`}
                  ...
                  <div className="space-y-2">
                    <label
                      className="text-sm font-medium"
                      htmlFor={`add-revenue-amount-${idx}`}
                    >
                      Amount
                    </label>
                    <Input
                      id={`add-revenue-amount-${idx}`}
                      inputMode="numeric"
                      value={row.amount}
                      onChange={(e) =>
                  ...
                  <div className="space-y-2">
                    <label
                      className="text-sm font-medium"
                      htmlFor={`add-revenue-ref-${idx}`}
                    >
                      Reference Number
                    </label>
                    <Input
                      id={`add-revenue-ref-${idx}`}
          ...
          {showDateAndUpload ? (
            <div className="space-y-2">
              <label className="text-sm font-medium" htmlFor="add-revenue-file">
