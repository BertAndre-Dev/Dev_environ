...
export type AdminAddressFilterOption = { label: string; value: string };

const ALL_ADDRESSES_OPTION: AdminAddressFilterOption = {
  label: "Addresses",
  value: "all",
};
...
export const getAdminEnergyConsumptionAddressOptions = createAsyncThunk(
  "admin-energy-consumption/getAddressOptions",
  async ({ estateId }: { estateId: string }, { rejectWithValue }) => {
    try {
      ...
      const entriesById = new Map<string, AdminAddressFilterOption>();
...
/** GET /api/v1/meters/estate/{estateId}/vend-analytics/chart (admin / estate overview) */
export const getAdminEnergyConsumptionChart = createAsyncThunk(
  "admin-energy-consumption/getChart",
  async (
    {
      estateId,
      period = "weekly",
