...
export type CompanyAddressFilterOption = { label: string; value: string };

const ALL_ADDRESSES_OPTION: CompanyAddressFilterOption = {
  label: "Addresses",
  value: "all",
};
...
export const getCompanyEnergyConsumptionAddressOptions = createAsyncThunk(
  "company-energy-consumption/getAddressOptions",
  async ({ estateId }: { estateId: string }, { rejectWithValue }) => {
    try {
      ...
      const entriesById = new Map<string, CompanyAddressFilterOption>();
...
/** GET /api/v1/meters/estate/{estateId}/vend-analytics/chart (company overview) */
export const getCompanyEnergyConsumptionChart = createAsyncThunk(
  "company-energy-consumption/getChart",
  async (
    {
      estateId,
      period = "weekly",
