...
export interface EstateAdminEnergyUsageJobMeta {
  jobId?: string;
  progress?: number;
  status?: string;
  source?: string;
...
export interface EstateAdminEnergyUsageResult {
  usage: EstateEnergyUsageData;
  meta: EstateAdminEnergyUsageJobMeta;
}

function extractJobMeta(value: unknown): EstateAdminEnergyUsageJobMeta {
...
export const getEstateAdminEstateEnergyUsage = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      const result: EstateAdminEnergyUsageResult = {
        usage: {
          ...usage,
