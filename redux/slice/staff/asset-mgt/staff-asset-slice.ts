...
export interface StaffAssetState {
  // Categories
...
const initialState: StaffAssetState = {
  categories: [],
...
const staffAssetSlice = createSlice({
  name: "staffAsset",
  initialState,
  reducers: {
    clearStaffAssetError: (state) => {
      state.error = null;
    },
...
export const { clearStaffAssetError, clearCurrentAsset } =
  staffAssetSlice.actions;
export default staffAssetSlice.reducer;

export const selectStaffAssetState = (state: RootState): StaffAssetState =>
  (state.staffAsset as StaffAssetState | undefined) ?? initialState;
