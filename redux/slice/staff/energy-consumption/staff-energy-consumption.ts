...
export type StaffAddressFilterOption = { label: string; value: string };

const ALL_ADDRESSES_OPTION: StaffAddressFilterOption = {
  label: "Addresses",
  value: "all",
};
...
export const getStaffEnergyConsumptionAddressOptions = createAsyncThunk(
  "staff-energy-consumption/getAddressOptions",
  async ({ estateId }: { estateId: string }, { rejectWithValue }) => {
    try {
      ...
      const entriesById = new Map<string, StaffAddressFilterOption>();
...
export const getStaffEnergyConsumptionChart = createAsyncThunk(
  "staff-energy-consumption/getChart",
  async (
    {
      estateId,
      period = "weekly",
